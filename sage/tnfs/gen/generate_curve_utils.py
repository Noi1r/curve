...
def find_min_max_list_mod_m(poly_p, length_p_min, length_p_max, m=1, umodm=[0]):
    """
    find the minimal u_min and maximal u_max such that
        poly_p(u_min) has length_p_min bits and u_min is minimal, and
        poly_p(u_max) has length_p_max bits and u_max is maximal.
    Allow length_p_min to be 0: start with the smallest possible u_min > 0 so that p is positive
    """
    if length_p_min == 0:
        umodm.sort()
        umin_pos = min(umodm)
        index = 0
        while poly_p(umin_pos) < 0:
            if index < len(umodm):
                index += 1
                umin_pos = (umin_pos % m) + umodm[index]
            else:
                index = 0
                umin_pos = (umin_pos % m) + m + umodm[0]
        umodm_ = [ui-m for ui in umodm]
        umodm_.sort()
        umin_neg = max(umodm_) # so the smallest in absolute value
        index = 0
        while poly_p(umin_neg) < 0:
            if index < len(umodm_):
                index += 1
                umin_neg = (umin_neg % m) + m + umodm_[index] # a negative number will have modulus value below
            else:
                index = 0
                umin_neg = (umin_neg % m) + umodm_[0]
        umin = [umin_pos, umin_neg]
    else:
    ...
    if length_p_min > 0:
        ...
        assert ZZ(poly_p(ZZ(u_min_pos-m))).nbits() < length_p_min
        assert ZZ(poly_p(ZZ(u_max_neg))).nbits() == length_p_min
        assert ZZ(poly_p(ZZ(u_max_neg+m))).nbits() < length_p_min

    assert ZZ(poly_p(ZZ(u_max_pos))).nbits() == length_p_max
    assert ZZ(poly_p(ZZ(u_max_pos+m))).nbits() > length_p_max

    assert ZZ(poly_p(ZZ(u_min_neg))).nbits() == length_p_max
    assert ZZ(poly_p(ZZ(u_min_neg-m))).nbits() > length_p_max
...
def get_next_seed(u_start, u_stop, m, u_mod_m, poly_p, poly_r, rnbits, allow_cofact_r=False, polys_cofact_twist=[],verbose=False,allowed_cofactor=1,factor_r=False,k=None):
    ...
    while (cond) and ((u_start <= u_stop and u_start <= u0 and u0 <= u_stop) or (u_start >= u_stop and u_start >= u0 and u0 >= u_stop)):
        ...
        try:
            ...
            print("u={} = {:#x}".format(u, u))
            print("p(u) = {}".format(poly_p(u)))
            print(err)
            raise err
        r = ZZ(poly_r(u))
