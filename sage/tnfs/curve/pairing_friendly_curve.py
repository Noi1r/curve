...
def compute_a_b(D, p, tr, y=None, verbose=False):
    ...
    if D in [2, 7, 11, 19, 43, 67, 163]:
        ...
        a0, b0 = a, b
        while not ok:
            # find a non-square w0 (w0 = w0p mod p) so that it will define a map to the quadratic twist and solve the issue of landing on the wrong twist
    ...
    while ab is None:
        ...
        while ab is None and j_idx < len(rH):
            ...
            # (a,b) ~ (a/u^4, b/u^6)
            # u = sqrt(3)/2:
            # y'^2 = x^3 - 3*j/(j - 1728)*x + 2*j/(j - 1728)
            # u = (j-1728)^(1/2):
            # y'^2 = x^3 - 3*j*(j - 1728)*x + 2*j*(j - 1728)^2 to avoid handling fractions
            j = Fp(rH[j_idx][0])
            j_idx += 1
            if j == 0:
