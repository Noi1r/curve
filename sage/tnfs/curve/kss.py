...
See also gg.py for other similar curves
"""
...
def polynomial_params(k):
    """
    Kachisa-Schaefer-Scott curves eprint 2007/452 and PAIRING'2008
    INPUT:
    - `k`: curve embedding degree, possible values are 8, 16, 18, 20, 32, 36, 40, 54
    - `choice`: which one of the family for the given embedding degree,
             for k=18, =20, there are two possible families.

    RETURN: px, rx, tx, cx, yx, betax, lambx, D

    from tnfs.curve.kss import polynomial_params as kss
    for k in [8,16,18,32,36,40,54]:
        print("\nKSS-{}".format(k))
        px, rx, tx, cx, yx, betax, lambrx, D = kss(k)
    ...
    if k == 8:
        ...
        lambx = (x**10 + 3116)/237
        m = 2370
        u_mod_m = [415, 1165, 1205, 1955]
        D = 1

    elif k == 54:
        # the new Scott-Guillevic k=54 curve, see also aurifeuillean.py
...
def poly_cofactor_twist_g1_g2(k, choice=None, D=None):
    QQx = QQ['x']; (x,) = QQx._first_ngens(1)
    px, rx, tx, cx, yx, betax, lambx, D = polynomial_params(k)
    twx = px+1+tx
    if k == 8:
...
def congruence_constraints(k, choice=None, D=None):
    if k == 8:
        m = 30
        u_m = [5, 25]
    elif k == 16:
...
class KSS(EllipticCurve_finite_field):
    ...
    def __init__(self, k, u, a=None, b=None, cofactor_r=None, verbose_init=False):
        ...
        - `cofactor_r`: a cofactor in r=R_KSS_k(u) (if not given and r not prime, raise error)
        """
