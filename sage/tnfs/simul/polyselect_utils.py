...
def write_tab_h(tab_h, deg, max_coeff, filename, header="", with_zeta=False, with_aut=False):
    """
    Write the table of precomputed polynomials h of fixed degree,
    with 1/zeta_Kh(2), w=number of roots of unity including 1, -1
    """
    ...
    if with_zeta:
        for i in range(len(tab_h)):
            ...
            if with_aut:
                ...
                aut = len(Kh.automorphisms())
                out_file.write("    ({:.8f},{:2d},{},{}),#{}\n".format(float(inv_z), int(w), aut, hc_string, h_string))
            else:
...
def automorphism_factor(h_coeffs):
    """
    return the possible degree of the automorphism. Can identify:
    - x -> 1/x when h is a palindrome
    - x -> -1/x when h is anti-palindrome
    - x -> -x when h is even, can be combined with palindrome

    TODO: identify when h is cyclotomic.
    The maximum degree is deg(h) * k1 / gcd(deg h, k1) where k = deg(h) * k1
    and is made possible only when f, g are fixed by the automorphisms of h.
    For that, either gcd(deg(h), k1) == 1 and f, g don't have coefficients in y if h has variable y,
    or the coefficients of f, g are made of trace_sigma(y) = y + sigma(y) + ... + sigma^(ord-1)(y)
    """
